...
use kerosene::{
    ...
    library::io::{
        ...
        io_pump::{close_descriptor, open_file, read, write},
    },
    main,
};
...
async fn main_actor() -> Exit {
    ...
    {
        println!("Opening file");
        let file = open_file("test.txt").await;

        println!("Reserving buffer");
        let mut buffer = reserve_buffer(0x1000).await;
        buffer.copy_from_slice("Hello, World!".as_bytes());

        println!("Writing file");
        let _ = write(file, 0, buffer).await;
